...
float3 prefilterEnvMap(float3 R, float3 N, float roughness)
{
    float3 V = R;
    ...
    uint2 texDim;
    int levels;
    textureCubeMap.GetDimensions(0, texDim.x, texDim.y, levels);
    
    float envMapDim = float(texDim.x);
    ...
    for (uint i = 0u; i < consts.numSamples; i++)
    {
        ...
        if (dotNL > 0.0)
        {
            ...
            color += textureCubeMap.SampleLevel(samplerCubeMap, L, mipLevel).rgb * dotNL;
            totalWeight += dotNL;
...
{		
    ...
    float3 color = prefilterEnvMap(N, N, consts.roughness);
