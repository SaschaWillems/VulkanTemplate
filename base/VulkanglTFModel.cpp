...
void vkglTF::Texture::fromglTfImage(tinygltf::Image &gltfimage, std::string path, vks::VulkanDevice *device, VkQueue copyQueue)
{
	...
	if (!isKtx) {
		...
		// @todo: this should depend on the type of image (base color, metallic roughness, etc.) as per glTF spec
#if defined(SRGB)
		...
		for (uint32_t i = 1; i < mipLevels; i++) {
			...
			int32_t w = int32_t(width >> (i - 1));
			int32_t h = int32_t(height >> (i - 1));
